...
export class OrderClient {
	...
	getOrdersByRestaurant(
		...
		return this.http.get<Order[]>(
			`${this.baseUrl}/${restaurantId}?ownerId=${ownerId}`
		);
	}
	...
	getOrdersByRestaurantAndCustomer(
		...
		return this.http.get<Order[]>(
			`${this.baseUrl}/${restaurantId}/customer/${customerId}?ownerId=${ownerId}`
		);
	}

	createOrder(customerId: number, restaurantId: number): Observable<Order> {
		return this.http.post<Order>(
			`${this.baseUrl}/restaurant/${restaurantId}/customer/${customerId}`,
			null
		);
	}
